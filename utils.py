...
def generate(
    ...
    with torch.no_grad(), autocast("cuda"), model.ema_scope():
        ...
        #input_image = 2 * torch.tensor(np.array(image))/ 255 - 1
        ...
        z_0 = torch.randn_like(cond["c_concat"][0]).to(model.device,dtype=dtype) * sigmas[0]
        z_1 = torch.randn_like(cond["c_concat"][0]).to(model.device,dtype=dtype) * sigmas[0]
